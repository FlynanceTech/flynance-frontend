...
export default function PlanejamentoIndexPage() {
  return (
    <AdvisorGuard>
      <PlanejamentoIndex />
    </AdvisorGuard>
  ...
  return (
    <section className="w-full px-4 pb-28 pt-6 md:px-6 lg:pb-8 lg:pt-8">
      <div className="mb-6">
        ...
        <p className="mt-2 text-sm text-slate-500">Selecione um cliente para editar o planejamento orçamentário.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 py-16">
          ...
          <p className="mt-3 text-sm font-semibold text-slate-500">Nenhum cliente encontrado</p>
        </div>
      ) : (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((client) => (
            ...
            >
              ...
              <SlidersHorizontal className="ml-auto h-4 w-4 shrink-0 text-[#4F98C2]" />
            </button>
          ))}
        </div>
      )}
