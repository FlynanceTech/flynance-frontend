...
function LoginContent() {
  ...
  useEffect(() => {
    ...
    const savedIdentifier = window.sessionStorage.getItem(STORAGE_IDENTIFIER_KEY)
  ...
  const handleSendCode = async (e: React.FormEvent) => {
    ...
    if (!identifier.trim()) {
      setError(method === 'email' ? t('errors.missingEmail') : t('errors.missingWhatsapp'))
      return
    }
  ...
  const handleComplete = (nextCode: string) => {
    setCode(nextCode)
  }
  ...
  const currentMethodLabel = method === 'email' ? t('methods.emailLabel') : t('methods.whatsappLabel')
      ...
      <section className="relative w-full h-full bg-gradient-to-r from-secondary to-primary flex items-center justify-center px-8">
        ...
        <div className="flex flex-col items-center z-20 text-center">
          <div className="flex flex-col gap-4 lg:gap-8 items-center max-w-[500px] lg:pt-4">
            ...
            <h1 className="text-xl lg:text-3xl font-bold text-white">{t('heroTitle')}</h1>
            ...
            <div className="flex-col gap-4 lg:gap-8 items-center hidden lg:flex">
              <span className="text-sm font-light text-white">{t('noAccount')}</span>
              <Link
                href="/cadastro/checkout?plano=essencial-mensal"
                className="border border-white text-white py-2 px-8 lg:py-4 lg:px-16 rounded-full text-base lg:text-xl hover:bg-white hover:text-primary transition-all"
              >
                {t('createNow')}
              </Link>
            </div>
        ...
        <div className="flex flex-col gap-8 items-center justify-center w-full h-full bg-white rounded-t-[48px] lg:rounded-[64px]">
          ...
          >
              ...
              <div className="pt-8 flex flex-col gap-8 w-full h-full">
                <div className="flex flex-col gap-4 items-center justify-center">
                  ...
                  <div className="flex gap-4 items-center w-full">
                    ...
                    >
                      <WhatsappIcon
                        className={clsx('w-5 h-5', method === 'whatsapp' ? 'text-white' : 'text-primary')}
                        fill={method === 'whatsapp' ? '#fff' : '#0065A4'}
                      />
                      {t('methods.whatsappLabel')}
                    </button>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <p className="text-sm text-gray-500 w-full mx-auto">
                    {t('sendCodeHint', { method: currentMethodLabel })}
                  </p>
                </div>
              </div>
            ) : (
              <>
                <h2 className="font-semibold text-xl text-[#333C4D] mt-4">
                  {t('typeCodeTitle', { method: currentMethodLabel })}
                </h2>
                ...
                <div className="flex gap-4 w-full">
                  ...
                  >
                    {t('switchMethod', {
                      method: method === 'email' ? t('methods.whatsappLabel') : t('methods.emailLower'),
                    })}
                  </button>
                  <button
                    type="button"
                ...
                >
                  {loading ? t('verifying') : isPending ? t('redirecting') : t('enter')}
                </button>
              </>
            )}
            ...
            <div className="flex-col gap-4 lg:gap-8 items-center lg:hidden flex">
              <span className="text-sm font-light">{t('noAccount')}</span>
              <Link
                href="/cadastro/checkout?plano=essencial-mensal"
                className="rounded-full text-xs hover:bg-white hover:text-primary transition-all"
              >
                {t('createNow')}
              </Link>
            </div>
            ...
            <div className="flex flex-col gap-4 items-center pb-4">
              <h3 className="text-lg lg:text-xl text-primary">{t('followUs')}</h3>
...
function LoginFallback() {
  ...
  return (
    <main className="w-screen h-screen flex items-center justify-center bg-white">
      <p className="text-sm text-slate-600">{t('loadingLogin')}</p>
    </main>
  )
}
