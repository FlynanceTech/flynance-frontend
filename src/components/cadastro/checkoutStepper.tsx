...
export default function CheckoutStepper({
 ...
 useEffect(() => {
  ...
  // 🔹 Cria automaticamente um lead assim que nome, e-mail e telefone estiverem preenchidos
  useEffect(() => {
    if (leadCreated) return; // já criamos

    const hasMinData = form.name && form.email && form.phone;
    if (!hasMinData) return;

    // se já temos user da sessão, não precisamos criar
    if (user?.userData?.user) {
      setUserFly(user.userData.user);
      setLeadCreated(true);
      return;
    }
    ...
    const createLead = async () => {
      try {
        ...
        if (created?.user) {
          setUserFly(created.user);
          setLeadCreated(true);
        }
      } catch (err) {
        console.error("Erro ao criar lead:", err);
        // não mostramos erro na tela pra não travar o fluxo, só log mesmo
      }
    };

    createLead();
  }, [form.name, form.email, form.phone, leadCreated, user, createMutation]);

  // fallback: garante um user se por algum motivo o efeito ainda não criou
  const ensureUser = async (): Promise<UserDTO> => {
    if (userFly) return userFly;
    ...
    if (!form.name || !form.email || !form.phone) {
      throw new Error(
        "Preencha nome, e-mail e WhatsApp para continuar com a criação do usuário."
      );
    }
  ...
  const handleSubmit = async () => {
    if (step === 2) return; // já está no sucesso
    ...
    try {
      ...
      if (!form.name || !form.email || !form.phone || !form.cpf) {
        throw new Error("Preencha nome, e-mail, WhatsApp e CPF.");
      }
        ...
        {step !== 2 && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="col-span-1 lg:col-span-2 flex flex-col gap-4 bg-white rounded-md shadow-[0_20px_60px_rgba(15,23,42,0.20)] border border-slate-200 p-6 md:p-8">
              ...
              {/* Cartão de crédito (sempre habilitado, independente do step) */}
              <div className="bg-white rounded-lg border border-gray-200 p-4 flex flex-col gap-4">
